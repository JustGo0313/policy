...
**Aquabook 不蒐集任何資料。** Aquabook 是一款水族缸記錄工具，沒有帳號，我們也沒有任何伺服器。你記錄的內容留在你自己的裝置上；若你保持備份開啟，也會存在**你自己的雲端帳號**裡（iPhone 與 iPad 上是 iCloud，Android 上是你的 Google 帳號），只有你讀得到。

### 資訊蒐集與使用
Aquabook **不會**蒐集、儲存或傳輸任何可識別個人身分的資訊。我們不會索取你的姓名、電子郵件、位置、聯絡人或照片。本 App 不需要註冊，也沒有帳號。

### 你的資料存放在哪裡
- 你記錄的一切——缸體、水質參數、生物、設備、維護事件與提醒——都儲存在**你自己的裝置上**，只有透過下面說明的備份才會被複製出去。那些備份都進到**屬於你的帳號**，沒有一份會進到我們這裡。
...
- **Android 系統備份**：在 Android 上，Aquabook 的紀錄會被納入作業系統本身的自動備份（Auto Backup），複製到**你的 Google 帳號**。那是由 Android 執行的，不是 Aquabook——我們不會收到也看不到它。你可以在 Android「設定 › Google › 備份」關閉，或在「設定 › 系統 › 備份」逐個 App 關閉。你的 Aquabook Pro 解鎖狀態**刻意被排除**在那份備份之外。
- 資料存放在本 App 專屬的私有儲存區，受作業系統的 App 沙盒與裝置加密保護。Aquabook 本身不另外再加一層加密。唯一的例外是你自己匯出的檔案——見〈匯出你的資料〉。

### 第三方服務
Aquabook **不含廣告、不含分析工具，也沒有任何會蒐集資料的第三方 SDK**。不會有任何第三方從本 App 取得資訊。

本 App 僅使用下列開源函式庫，它們都不會傳輸你的資料：本地儲存（`hive_ce`、`hive_ce_flutter`）、識別碼產生（`uuid`）、檔案位置查詢（`path_provider`）、App 內購買介面（`in_app_purchase`）、Android 上的裝置端提醒（`flutter_local_notifications`、`timezone`、`flutter_timezone`），以及 Apple 的標準圖示集（`cupertino_icons`）。

### 網路存取
Aquabook 不會連到我們的伺服器——我們根本沒有。唯一的網路行為是上面說的那些備份（由作業系統處理，進的是你自己的 iCloud 或 Google 帳號），以及你購買或還原時的 Apple App Store 或 Google Play。

### 購買
Aquabook Pro 是一次性買斷，完全由 Apple 處理。**我們不會看到也不會收到你的付款資訊。** 本 App 僅在裝置上儲存一個「是否已解鎖 Pro」的旗標。購買與還原都透過 Apple 的 App Store 系統進行，其交易處理方式請參閱 Apple 的隱私政策。

### 通知
提醒功能是**選用的**。當你建立提醒時，Aquabook 會向作業系統請求發送本地通知的權限。這些通知**完全在你的裝置上產生與發送**，不會傳送到我們或任何伺服器。你隨時可以在裝置的設定中關閉（在 Android 上，提醒還需要「鬧鐘和提醒」這道權限）。

### 匯出你的資料
Aquabook 可以把你的資料匯出成 JSON 備份或 CSV 檔案，此功能免費。匯出的檔案在**你的裝置上**產生，並由你透過裝置的標準分享面板決定要傳到哪裡。檔案一旦被你傳送到其他地方，即適用該目的地本身的隱私政策。我們不會收到任何副本。

**在 Android 上，匯出還會在你的「Downloads」資料夾留一份副本。** 那是你一按下匯出就發生的——在你選擇要分享到哪裡之前，而且**就算你接著取消分享，那一份仍然留著**。它位在共用儲存空間，所以其他有儲存權限的 App 讀得到它；解除安裝 Aquabook 不會移除它，在 App 裡刪掉一個缸也不會刪掉它。不想留著的話，請用檔案管理程式自行刪除。

### 兒童隱私
Aquabook 不會蒐集任何年齡層使用者的資料，包括 13 歲以下的兒童。

### 本隱私政策的變更
本政策可能不定期更新。任何變更都會公布在本頁，並更新上方的「最後更新」日期。

### 聯絡我們
如果你對本隱私政策有任何疑問，請透過 **lkk54545@gmail.com** 與我們聯絡。
