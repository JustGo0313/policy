...
**Aquabook 不蒐集任何資料。** Aquabook 是一款水族缸記錄工具，沒有帳號，我們也沒有任何伺服器。你記錄的內容留在你自己的裝置上；若你保持備份開啟，也會存在**你自己的 iCloud 帳號**裡，只有你讀得到。
...
本 App 僅使用下列開源函式庫，它們都不會傳輸你的資料：本地儲存（`hive_ce`）、識別碼產生（`uuid`）、檔案位置查詢（`path_provider`），以及 Apple 的 App 內購買介面（`in_app_purchase`）。

### 網路存取
Aquabook 不會連到我們的伺服器——我們根本沒有。唯一的網路行為是上面說的 iCloud 備份（由作業系統處理，進的是你自己的 iCloud 帳號），以及你購買或還原時的 Apple App Store。
...
### 通知
提醒功能是**選用的**。當你建立提醒時，Aquabook 會向作業系統請求發送本地通知的權限。這些通知**完全在你的裝置上產生與發送**，不會傳送到我們或任何伺服器。你隨時可以在 iOS 設定中關閉。

### 匯出你的資料
Aquabook 可以把你的資料匯出成 JSON 備份或 CSV 檔案，此功能免費。匯出的檔案在**你的裝置上**產生，並由你透過 iOS 標準分享面板決定要傳到哪裡。檔案一旦被你傳送到其他地方，即適用該目的地本身的隱私政策。我們不會收到任何副本。
